...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const indexableProducts = products.filter((product) => !product.seo.noIndex);
  const indexablePosts = posts.filter((post) => !post.seo.noIndex);

  const latestPostDate = indexablePosts.reduce((latest, post) => {
    const timestamp = new Date(post.updatedAt || post.publishedAt).getTime();
    return Number.isNaN(timestamp) ? latest : Math.max(latest, timestamp);
  }, 0);
  const latestProductDate = indexableProducts.reduce((latest, product) => {
    const timestamp = new Date(productLastModifiedMap[product.slug] || "").getTime();
    return Number.isNaN(timestamp) ? latest : Math.max(latest, timestamp);
  }, 0);
  ...
  const productRoutes = indexableProducts.map((product) => ({
  ...
  const blogRoutes = indexablePosts.map((post) => ({
